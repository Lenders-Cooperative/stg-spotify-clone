...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        genres = (
            Genre.objects.all()
            .select_related("songs", "songs__artist", "songs__album")
            .values(
                "name",
                no_of_songs=Count("songs", distinct=True),
                no_of_artists=Count("songs__artist", distinct=True),
                no_of_albums=Count("songs__album", distinct=True),
            )
        )
        genre_chart_labels = json.dumps([genre["name"] for genre in genres])
        context["genre_charts"] = [
            {"id": "songs_per_genre", "labels": genre_chart_labels, "label": "# Songs per Genre", "data": [genre["no_of_songs"] for genre in genres]},
            {
        ...
        play_logs = (
            SongPlayLog.objects.select_related("song")
            .filter(date_played__month=timezone.now().month)
            .values(artist_name=F("song__artist__name"))
            .annotate(no_of_plays=Count("*"))
            .order_by("-no_of_plays")[:10]
        )
